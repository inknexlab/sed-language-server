...
import { completions } from "../../src/analysis/completion.js";
import { offsetAt, positionAt, withAnalysisSnapshot } from "./helpers.js";
...
function completionItems(snapshot, position) {
  return completions(snapshot, offsetAt(snapshot.source, position)).map(
    ({ startOffset, endOffset, ...item }) => ({
      ...item,
      editRange: {
        start: positionAt(snapshot.source, startOffset),
        end: positionAt(snapshot.source, endOffset),
      },
    }),
  );
}
...
function assertCommandItems(items, verbs, editRange) {
  ...
  for (const completion of items) {
    assert.equal(completion.category, "command");
    assert.equal(typeof completion.detail, "string");
    assert.equal(typeof completion.documentation.synopsis, "string");
    assert.equal(typeof completion.documentation.description, "string");
    assert.deepEqual(completion.editRange, editRange);
    assert.equal(completion.newText, completion.label);
  }
}

function substitutionFlagItems(labels, editRange) {
  return substitutionFlags
    .filter(({ label }) => labels.includes(label))
    .map((reference) => ({
      category: "substitutionFlag",
...
test("returns source-offset completion edits and validates public arguments", async () => {
  await withSnapshot("bre", ":target\nb \n", (snapshot) => {
    assert.deepEqual(completions(snapshot, 10), [
      {
        category: "label",
        startOffset: 10,
        endOffset: 10,
    ...
    assert.throws(() => completions(snapshot, 0.5), /must be an integer/);
    assert.throws(() => completions(snapshot, -1), /outside the source/);
  });
});
...
test("completes every POSIX command verb at an empty script", async () => {
  for (const mode of ["bre", "ere"]) {
    await withSnapshot(mode, "", (snapshot) => {
      ...
      assert.deepEqual(items.find(({ label }) => label === "a").documentation, {
        synopsis: "[address]a\\\ntext",
        description:
...
test("returns presentation-neutral command documentation", async () => {
  await withSnapshot("bre", "", (snapshot) => {
    const item = completionItems(snapshot, {
      line: 0,
      character: 0,
    }).find(({ label }) => label === "D");
...
test("returns presentation-neutral substitution flag documentation", async () => {
...
test("does not reinterpret missing-separator issues as command slots", async () => {
  const cases = [
...
test("keeps repeated completion local in a large command list", {
  timeout: 3000,
}, async () => {
  const source = "p\n".repeat(50_000);
  await withSnapshot("bre", source, (snapshot) => {
    ...
    for (let request = 0; request < 20; request += 1) {
      assert.equal(
        completions(snapshot, source.length).length,
        allCommandVerbs.length,
      );
    }
...
test("filters command verbs after a complete address prefix", async () => {
  const cases = [
    {
      source: "1",
...
test("keeps an incomplete nested command's address count at a block boundary", async () => {
  const cases = [
    {
      source: "{1}",
...
test("does not complete after an invalid or incomplete command prefix", async () => {
  const cases = [
    { source: "1,", position: { line: 0, character: 2 } },
    { source: ",2", position: { line: 0, character: 2 } },
    { source: "1 2", position: { line: 0, character: 3 } },
    { source: "1!!", position: { line: 0, character: 3 } },
    { source: "1! ", position: { line: 0, character: 3 } },
